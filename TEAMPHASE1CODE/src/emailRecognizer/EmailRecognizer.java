...
public class EmailRecognizer {
	...
	private static int TLDCounter = 0; 					// TLD is between 2 and 6 characters
	private static boolean TLD = false;						// TLD part of the domain
	...
	public static String checkEmailAddress(String input) {
		...
		while (running) {
			...
			case 1: 
				...
				// there can't be consecutive zeros or hyphens 
				else if ((currentChar == '.' || currentChar == '-')) {
					nextState = 0;
				}
				// transition into the domain part
				else if ((currentChar == '@')) {
					nextState = 2;
				}
				else {
			...
			case 2: // transition to domain part
				// if an alphanumeric char, go to state 3. if TLD is true, we are in TLD and only accept A-Z
				...
				if (TLD) {
					...
					else { // only letters are allowed in TLD
						running = false;
					}
			...
			case 3:
				// EoI: Domain part cannot be longer than 63 digits
				if (domainPartCounter > 63) { 
					running = false;
				}
				
				// EoI: TLD cannot be longer than 6 digits. 
				if (TLDCounter > 6) { //TLD is too long
					running = false;
				}
				
				// if in TLD, only letters are allowed
				if (TLD) {
					if ((currentChar >= 'A' && currentChar <= 'Z') ||
							(currentChar >= 'a' && currentChar <= 'z')) 
					{
					...
					else if (currentChar == '.') { //another TLD, so reset the counter and go back to state 2
						TLDCounter = 0;
						nextState = 2;
					}
					else { // only letters are allowed in TLD
				...
				// if not in TLD, we are in the domain part, so letters and digits are allowed.
				else if (!TLD) {
					if (currentChar >= 'A' && currentChar <= 'Z' ||
							(currentChar >= 'a' && currentChar <= 'z') ||
							(currentChar >= '0' && currentChar <= '9')) {
					...
					// case 3, TLD becomes true and go to state 2
					else if (currentChar == '.' ) {
						TLD = true;
						nextState = 2;
					}
					// case 4, go to state 4
					else if (currentChar == '-') {
						nextState = 4;
			...
			case 4: 
				...
				// if input is a char, go to state 3 
				if (currentChar >= 'A' && currentChar <= 'Z' ||
				(currentChar >= 'a' && currentChar <= 'z') ||
				(currentChar >= '0' && currentChar <= '9')) {
					domainPartCounter++;
		...
		case 1:
			...
			emailAddressErrorMessage = "Invalid input, may only be alphanumeric.\n";
			return emailAddressErrorMessage;
		...
		case 2:
			...
			emailAddressIndexofError = currentCharNdx; 
			emailAddressErrorMessage = "Domain part cannot end with '.'.\n";
			return emailAddressErrorMessage;
		...
		case 3:
			...
			if (domainPartCounter < 2) {
				emailAddressIndexofError = currentCharNdx;
				emailAddressErrorMessage = "Top Level Domain must be between 2 and 63 characters.\n";
				return emailAddressErrorMessage;
			}
			if (TLDCounter < 2) {
				emailAddressIndexofError = currentCharNdx;
				emailAddressErrorMessage = "TLD must be between 2 and 6 characters";
				return emailAddressErrorMessage;
			}
			else 
			{
		...
		case 4:
			...
			emailAddressIndexofError = currentCharNdx; 
			emailAddressErrorMessage = "Domain part cannot end with a hyphen.\n";
			return emailAddressErrorMessage;
